...
import { createFileFilter as defaultFileFilterCreator, FileFilter } from "../util/file-filter";
// Import flow types.
...
export async function getDefaultLocalizedName({ messageFile, manifestData }: LocalizedNameParams): Promise<string> {
...
function getPackageNameFromTemplate(filenameTemplate: string, manifestData: ExtensionManifest): string {
...
export type PackageCreatorFn = (params: PackageCreatorParams) => Promise<ExtensionBuildResult>;
...
export async function defaultPackageCreator(
  ...
  if (manifestData) {
    ...
    console.log(`Using manifest id=${id || "[not specified]"}`);
  } else {
  ...
  try {
    ...
    console.log(`Destination exists, overwriting: ${extensionPath}`);
