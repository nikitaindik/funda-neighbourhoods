...
export type PreferencesGetterFn = (appName: PreferencesAppName) => FirefoxPreferences;

export function getPrefs(app: PreferencesAppName = "firefox"): FirefoxPreferences {
...
export function coerceCLICustomPreference(cliPrefs: Array<string>): FirefoxPreferences {
  const customPrefs = {};
  ...
  for (const pref of cliPrefs) {
    ...
    if (nonOverridablePreferences.includes(key)) {
      console.log(`'${key}' preference cannot be customized.`);
      continue;
    }
