...
export class Program {
  ...
  constructor(argv: ?Array<string>, { absolutePackageDir = process.cwd() }: ProgramOptions = {}) {
  ...
  command(name: string, description: string, executor: Function, commandOptions: Object = {}): Program {
  ...
  cleanupProcessEnvConfigs(systemProcess: typeof process) {
    ...
    const toOptionKey = k => decamelize(camelCase(k.replace(envPrefix, "")), { separator: "-" });
    ...
    if (cmd) {
      Object.keys(env)
        ...
        .forEach(k => {
          ...
          if (!globalOpt && !cmdOpt) {
            console.log(`Environment ${k} not supported by web-ext ${cmd}`);
            delete env[k];
          }
        });
    }
  ...
  async execute({
    ...
    try {
      ...
      if (argv.configDiscovery) {
        ...
        console.log("Not discovering config files");
      }
      ...
      if (configFiles.length) {
        const niceFileList = configFiles
          ...
          .join(", ");
        console.log("Applying config file" + `${configFiles.length !== 1 ? "s" : ""}: ` + `${niceFileList}`);
      }
...
export function defaultVersionGetter(
  ...
  if (globalEnv === "production") {
    ...
    const packageData: any = readFileSync(path.join(absolutePackageDir, "package.json"));
...
export function main(
  absolutePackageDir: string,
  { getVersion = defaultVersionGetter, commands = defaultCommands, argv, runOptions = {} }: MainParams = {}
): Promise<any> {
  ...
  program
    .command("build", "Create an extension package from source", commands.build, {
      ...
      filename: {
        ...
        requiresArg: true,
        type: "string",
        coerce: throwUsageErrorIfArray("Multiple --filename/-n option are not allowed"),
      },
      "overwrite-dest": {
        alias: "o",
