...
export default class ADBUtils {
  ...
  runShellCommand(deviceId: string, cmd: string | Array<string>): Promise<string> {
    const { adb, adbClient } = this;

    console.log(`Run adb shell command on ${deviceId}: ${JSON.stringify(cmd)}`);
  ...
  async discoverInstalledFirefoxAPKs(deviceId: string, firefoxApk?: string): Promise<Array<string>> {
    console.log(`Listing installed Firefox APKs on ${deviceId}`);
  ...
  async ensureRequiredAPKRuntimePermissions(deviceId: string, apk: string, permissions: Array<string>): Promise<void> {
  ...
  async detectOrRemoveOldArtifacts(deviceId: string, removeArtifactDirs?: boolean = false): Promise<boolean> {
    const { adbClient } = this;
    ...
    return wrapADBCall(async () => {
      ...
      for (const file of files) {
        ...
        console.log(`Removing artifacts directory ${artifactsDir} from device ${deviceId}`);
  ...
  async clearArtifactsDir(deviceId: string): Promise<void> {
    ...
    console.log(`Removing ${artifactsDir} artifacts directory on ${deviceId} device`);

    await this.runShellCommand(deviceId, ["rm", "-rf", artifactsDir]);
  }

  async pushFile(deviceId: string, localPath: string, devicePath: string): Promise<void> {
    const { adbClient } = this;

    console.log(`Pushing ${localPath} to ${devicePath} on ${deviceId}`);
  ...
  async startFirefoxAPK(deviceId: string, apk: string, apkComponent: ?string, deviceProfileDir: string): Promise<void> {
    const { adbClient } = this;

    console.log(`Starting ${apk} on ${deviceId}`);
  ...
  async setupForward(deviceId: string, remote: string, local: string) {
    ...
    console.log(`Configuring ADB forward for ${deviceId}: ${remote} -> ${local}`);
...
export async function listADBFirefoxAPKs(deviceId: string, adbBin?: string): Promise<Array<string>> {
  const adbClient = new ADBUtils({ adbBin });
